...
namespace osu.Server.OnlineDbGenerator
{
    public class Generator
    {
        ...
        /// </summary>
        private static string conditional = "approved IN (1, 2, 4)";
        ...
        private void copyBeatmapSets(IDbConnection source, IDbConnection destination)
        {
            ...
            var beatmapSetsReader = source.Query<BeatmapSetRow>($"SELECT beatmapset_id, approved, approved_date, submit_date FROM osu_beatmapsets WHERE {conditional}");
        ...
        private void copyBeatmaps(IDbConnection source, IDbConnection destination)
        {
            ...
            var beatmapsReader = source.Query<BeatmapRow>($"SELECT beatmap_id, beatmapset_id, user_id, filename, checksum, approved, last_update FROM osu_beatmaps WHERE {conditional}");
        ...
        private int getBeatmapSetCount(IDbConnection conn) => conn.QuerySingle<int>($"SELECT COUNT(beatmapset_id) FROM osu_beatmapsets WHERE {conditional}");

        /// <summary>
        /// Count beatmaps from MySQL or SQLite database.
        /// </summary>
        /// <param name="conn">Connection to fetch beatmaps from.</param>
        private int getBeatmapCount(IDbConnection conn) => conn.QuerySingle<int>($"SELECT COUNT(beatmap_id) FROM osu_beatmaps WHERE {conditional}");
